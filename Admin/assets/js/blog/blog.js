...
(function() {
    ...
    // ========================================
    // STATIC DATA FUNCTIONS (MOCK_DATA)
    // ========================================

    /**
     * Load blogs from MOCK_DATA
     */
    async function loadBlogs() {
        try {
            ...
            if (typeof MOCK_DATA === 'undefined' || typeof MOCK_DATA.getBlogPosts !== 'function') {
                throw new Error('MOCK_DATA not ready. Make sure data.js is loaded.');
            }

            let blogs = MOCK_DATA.getBlogPosts(100); // Get all
            ...
            if (searchQuery) {
                ...
                blogs = blogs.filter(b => 
                    ...
                    (b.judul && b.judul.toLowerCase().includes(query))
                );
            }

            renderBlogs(blogs);
            renderPagination({ total_pages: 1, current_page: 1 });
    ...
    function renderBlogs(blogs) {
        ...
        const blogsHTML = blogs.map((blog, index) => {
            const blogTitle = blog.title || blog.judul || 'Tanpa Judul';
            // Use MOCK_DATA date format or default
            const updatedDate = formatDate(blog.date || blog.created_at || new Date().toISOString());
            ...
            return `
                <div class="blog-item">
                    <div class="row g-0 align-items-center w-100 position-relative">
                        ...
                        <!-- Title Column -->
                        <div class="col-lg-5 col-title"><span class="blog-title">${escapeHtml(blogTitle)}</span></div>
    ...
    /**
     * Handle delete button click (MOCKED)
     */
    async function handleDelete() {
        ...
        try {
            ...
            // Find and remove elements from DOM directly to simulate deletion
            checkedIds.forEach(id => {
               const checkbox = document.querySelector(`.blog-checkbox[data-blog-id="${id}"]`);
               if(checkbox) {
                   const blogItem = checkbox.closest('.blog-item');
                   if(blogItem) blogItem.remove();
               }
            });
