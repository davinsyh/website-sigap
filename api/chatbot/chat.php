...
function determinePhase($intentResult) {
    ...
    $msgCount = $_SESSION['message_count'] ?? 0;
    $cumulScore = $_SESSION['cumulative_score'] ?? 0;
    
    // PENTING: Jangan buru-buru ke consent.
    // Biarkan user cerita dulu dengan leluasa.
    // Minimal 8 pesan (4 pertukaran) DAN skor kumulatif >= 7
    // sebelum menawarkan laporan.
    
    switch ($tier) {
        case TIER_REPORT:
            // Bahkan dengan sinyal kuat, tunggu minimal 6 pesan
            if ($msgCount >= 6 && $cumulScore >= INTENT_THRESHOLD_REPORT) {
                return 'consent';
            }
            return 'collect';
            
        case TIER_POTENTIAL:
            // Perlu lebih banyak percakapan sebelum tanya consent
            if ($msgCount >= 8 && $cumulScore >= INTENT_THRESHOLD_REPORT) {
                return 'consent';
            }
            return 'collect';
