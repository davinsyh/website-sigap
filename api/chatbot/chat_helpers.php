...
class ChatHelpers {
    ...
    /**
     * Klasifikasi sikon keraguan user
     * Digunakan ketika consent = 'no' untuk menentukan alasan keraguan
     * 
     * @param string $message
     * @return string 'fear' | 'pessimism' | 'fatigue' | 'unknown'
     */
    public static function classifyHesitation($message) {
        $msg = strtolower(trim($message));
        ...
        $pessimismKeywords = [
            ...
            'gak percaya', 'tidak percaya', 'buat apa',
            'gak ada hasilnya', 'buang waktu', 'hopeless',
        ...
        $fatigueKeywords = [
            ...
            'overwhelm', 'drain', 'pusing', 'sakit', 'luka', 'terluka'
        ];

        foreach ($fearKeywords as $kw) {
            if (strpos($msg, $kw) !== false) return 'fear';
        }

        foreach ($pessimismKeywords as $kw) {
            if (strpos($msg, $kw) !== false) return 'pessimism';
        }

        foreach ($fatigueKeywords as $kw) {
            if (strpos($msg, $kw) !== false) return 'fatigue';
        }
