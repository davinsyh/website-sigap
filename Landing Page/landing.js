...
(function () {
    ...
    // ============================================
    // FETCH STATISTICS FROM MOCK_DATA
    ...
    async function fetchStatistics() {
      try {
        if (typeof MOCK_DATA === 'undefined' || typeof MOCK_DATA.getStatistics !== 'function') {
           throw new Error('MOCK_DATA not ready');
        }
        
        const stats = MOCK_DATA.getStatistics();
        
        // Match the expected format from the old API: total_cases and cases_completed
        statisticsData = {
...
(function initKabarSigapCarousel() {
  ...
  // Articles Data (MOCK_DATA)
  let articlesData = typeof MOCK_DATA !== 'undefined' ? MOCK_DATA.getBlogPosts(5) : [];
  ...
  articlesData = articlesData.map(a => ({
    ...
    category: a.category || a.kategori || 'Berita',
    desc: a.desc || a.summary || '',
    image: a.image || a.gambar_header_url || 'https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=800&q=80',
    link: '#'
  }));
  ...
  function initDesktopCards() {
    ...
    articlesData.forEach((article, i) => {
      ...
      // No read-more button for view-only experience
  ...
  function updateDesktopCardStates() {
    ...
    cards.forEach((card) => {
      ...
      // Update cursor for non-active cards (allows bringing them to front, but not navigating)
  ...
  function renderMobileCarousel() {
    ...
    articlesData.forEach((article, idx) => {
      ...
      // Removed click to open article logic
