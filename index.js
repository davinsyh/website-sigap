(function () {
  ...
  const loadStatistics = async () => {
    try {
      if (
        ...
        typeof MOCK_DATA.getStatistics !== "function"
      ) {
        throw new Error("MOCK_DATA tidak ditemukan.");
      }
      ...
      const total_cases = stats.total;
      const cases_completed = stats.completed;

      // Dashboard stats
      updateElement("total-cases", total_cases);
      updateElement("cases-received", total_cases);
  ...
  if ("IntersectionObserver" in window) {
          ...
          if (entry.isIntersecting) {
            ...
            if (parent && !parent.dataset.staggered) {
              ...
              group.forEach((el, idx) => {
                el.style.transitionDelay = prefersReduced
                  ? "0ms"
                  : `${Math.min(idx * 100, 600)}ms`;
              });
    ...
    document
      .querySelectorAll(".stat-number, .transparansi-stat")
  ...
  /* --- BUTTON NAVIGATION --- */
  const laporButtons = document.querySelectorAll(
    ".js-lapor-nav, .js-lapor-hero, .js-lapor-monitoring, .js-lapor-about",
  );
