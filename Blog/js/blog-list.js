...
async function fetchBlogs() {
    ...
    setTimeout(() => {
        try {
            if (typeof MOCK_DATA === 'undefined' || !MOCK_DATA.getBlogPosts) {
                container.innerHTML = `<div class="error-state text-center" style="grid-column:1/-1">Gagal memuat berita: Data tidak tersedia.</div>`;
                return;
            }
            
            let allBlogs = MOCK_DATA.getBlogPosts();
            ...
            if (currentSearch) {
                ...
                allBlogs = allBlogs.filter(b => 
                    ...
                    (b.judul && b.judul.toLowerCase().includes(searchLower)) ||
                    (b.category && b.category.toLowerCase().includes(searchLower)) ||
                    (b.kategori && b.kategori.toLowerCase().includes(searchLower))
                );
            }
            ...
            // Convert formats to match old PHP keys since renderBlogs uses blog.judul, blog.created_at, etc
            const formattedBlogs = paginatedBlogs.map(b => ({
                id: b.id || b.slug,
                judul: b.title || b.judul,
                excerpt: b.excerpt || b.summary,
                created_at: b.date || b.created_at || '2025-01-01',
                kategori: b.category || b.kategori || 'Umum',
                gambar_header_url: b.image || b.gambar_header_url || CONFIG.DEFAULT_IMAGE
            }));
