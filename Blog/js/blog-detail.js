...
// Fetch artikel terkait dari MOCK_DATA
async function fetchRelated(currentId) {
    try {
        if (typeof MOCK_DATA === 'undefined' || !MOCK_DATA.getBlogPosts) return;

        const allBlogs = MOCK_DATA.getBlogPosts();
        // Remove current article and limit to 3
        const filtered = allBlogs.filter(b => b.id != currentId && b.slug != currentId).slice(0, 3);
        ...
        if (filtered.length > 0) {
            ...
            container.innerHTML = filtered.map(blog => {
                const img = fixImagePath(blog.gambar_header_url || blog.image);
                const title = blog.judul || blog.title;
                const date = formatDate(blog.created_at || blog.date);
                return `
                <div class="related-card" onclick="window.location.href='?id=${blog.id || blog.slug}'">
                    <img src="${img}" alt="${title}" class="related-image" onerror="this.src='${CONFIG.DEFAULT_IMAGE}'">
...
async function fetchDetail() {
    ...
    try {
        if (typeof MOCK_DATA === 'undefined' || !MOCK_DATA.getBlogPostById) {
            throw new Error('Data tidak tersedia');
        }

        const blogData = MOCK_DATA.getBlogPostById(id) || MOCK_DATA.blogPosts.find(b => b.slug === id);

        if (blogData) {
            // Mapping fields from new MOCK_DATA format to expected fields in renderArticle
            const mappedData = {
            ...
            renderArticle(mappedData);
            fetchRelated(mappedData.id);
            initShareButton();
        } else {
             document.getElementById('article-area').innerHTML = `
                <div class="error-container">
                    ...
                    <button onclick="window.location.href='${CONFIG.HOME_URL}'" class="btn btn-primary mt-20">Kembali ke Wawasan</button>
                </div>
             `;
        }
    } catch (error) {
        ...
        document.getElementById('article-area').innerHTML = `
            <div class="error-container">
                ...
                <p>Gagal memuat detail artikel. Silakan cek koneksi internet Anda.</p>
